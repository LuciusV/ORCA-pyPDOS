...
def ReadMOComposition(Content, LenContent, Type, MONumber):
    ...
    :return Species: dictionary of key(different atoms) -> value(AO min and max indices in list)
    ...
    for index in xrange(EndPage * (MONumber + 4)):
        ...
        if LineOnPage == 1:  # Eigenvalues
            Eigenvalues[Page * 6:Page * 6 + Columns] = [float(E) for E in Line.split()[0:Columns]]
            ...
            ContributionMatrix[Page * 6:Page * 6 + Columns, (LineOnPage - 4 + Page * MONumber) % MONumber] = \
                [float(E) for E in [Line[shift * 10 + 16: shift * 10 + 16 + 10] for shift in range(Columns)]]
            if Page == 0:
                Atom = str(Line[:5]).strip(' ')
                if Atom not in Species.keys():
                    Species[Atom] = [LineOnPage - 4, 0]
                else:
                    if LineOnPage - 4 > Species[Atom][1]:
                        Species[Atom][-1] = LineOnPage - 4
    ...
    if Type == 'UKS':
        ...
        for index in xrange(EndPage * (MONumber + 4)):
            ...
            if Page + 1 == EndPage:
                Columns = MONumber % 6
            if LineOnPage == 1:  # Eigenvalues
                EigenvaluesB[Page * 6:Page * 6 + Columns] = [float(E) for E in Line.split()[0:Columns]]
            elif LineOnPage == 2:  # Occupations
